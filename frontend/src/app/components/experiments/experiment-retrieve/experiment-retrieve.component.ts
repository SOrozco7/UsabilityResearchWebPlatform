...
export class ExperimentRetrieveComponent implements OnInit {
  ...
  listParticipants() {
    ...
    this.router.navigate(['experiments/' + this.id + '/participants']);
  }
  ...
  selectParticipant(experimentId: number) {

    this.router.navigate(['experiments/' + experimentId + '/participants/select']);
  }
}
